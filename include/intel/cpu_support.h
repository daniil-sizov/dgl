...
class ElemWiseUpdate : public Xbyak::CodeGenerator {
 ...
 public:
  ...
  template <class Operator, utils::Verify<Operator, ::dgl::aten::cpu::sddmm_op::Sub,
                                          supported_types> = true>
  void full_chunk_loop_operations() {
    typedef typename Operator::type IType;
    loop_pre<IType>();
  ...
  template <class Operator, utils::Verify<Operator, ::dgl::aten::cpu::sddmm_op::Div,
                                          supported_types> = true>
  void full_chunk_loop_operations() {
    typedef typename Operator::type IType;
    loop_pre<IType>();
...
// Fix it Iza - it is for MUL
  template <class Operator, utils::Verify<Operator, ::dgl::aten::cpu::sddmm_op::Dot,
                                          supported_types> = true>
  void full_chunk_loop_operations() {
    typedef typename Operator::type IType;
    loop_pre<IType>();
    alias_DOT<IType>(zmm2, zmm1, zmm2);
    //alias_ADD<DType>(zmm2, zmm0, zmm2);
    loop_post<IType>();
  }
  ...
  void remainder_operations(const Xbyak::Opmask mask) {
    typedef typename Operator::type IType;
    remainder_fetch_LR<IType>(mask);
    alias_DOT<DType>(zmm2, zmm2, zmm1);
    //alias_ADD<DType>(zmm2, zmm0, zmm2);
  }
